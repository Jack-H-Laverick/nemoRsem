// Generated by using Rcpp::compileAttributes() -> do not edit by hand
// Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

#include <RcppArmadillo.h>
#include <Rcpp.h>

using namespace Rcpp;

#ifdef RCPP_USE_GLOBAL_ROSTREAM
Rcpp::Rostream<true>&  Rcpp::Rcout = Rcpp::Rcpp_cout_get();
Rcpp::Rostream<false>& Rcpp::Rcerr = Rcpp::Rcpp_cerr_get();
#endif

// array_w_mean
NumericVector array_w_mean(arma::cube array, DataFrame scheme);
RcppExport SEXP _nemoRsem_array_w_mean(SEXP arraySEXP, SEXP schemeSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< arma::cube >::type array(arraySEXP);
    Rcpp::traits::input_parameter< DataFrame >::type scheme(schemeSEXP);
    rcpp_result_gen = Rcpp::wrap(array_w_mean(array, scheme));
    return rcpp_result_gen;
END_RCPP
}
// emptyRcpp
LogicalMatrix emptyRcpp(arma::cube a);
RcppExport SEXP _nemoRsem_emptyRcpp(SEXP aSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< arma::cube >::type a(aSEXP);
    rcpp_result_gen = Rcpp::wrap(emptyRcpp(a));
    return rcpp_result_gen;
END_RCPP
}

static const R_CallMethodDef CallEntries[] = {
    {"_nemoRsem_array_w_mean", (DL_FUNC) &_nemoRsem_array_w_mean, 2},
    {"_nemoRsem_emptyRcpp", (DL_FUNC) &_nemoRsem_emptyRcpp, 1},
    {NULL, NULL, 0}
};

RcppExport void R_init_nemoRsem(DllInfo *dll) {
    R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
    R_useDynamicSymbols(dll, FALSE);
}
